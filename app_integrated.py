"""
Citation Processor with Incipit Conversion - Integrated Version
Combines citation formatting and incipit note generation
"""
...
def extract_endnotes(path):
    ...
    for en in dom.getElementsByTagName('w:endnote'):
        ...
        if en_id and en_id not in ['-1', '0']:
            ...
            # For incipit: remove endnote number and leading punctuation
...
def create_notes_section_xml(endnotes, references, formatted=False):
...
def process():
    ...
    mode = request.form.get('mode', 'format')  # 'format', 'incipit', or 'both'
    ...
    try:
        ...
        if mode in ['format', 'both']:
            ...
            # Update endnotes.xml with formatting
        ...
        if mode in ['incipit', 'both']:
            ...
            notes_to_use = formatted_endnotes if formatted_endnotes else endnotes
            
            # Create notes section
            notes_xml = create_notes_section_xml(endnotes, references, formatted_endnotes)
